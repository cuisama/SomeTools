...
bg.mediaCss(document);
bg.showVersionUpdate();
...
function createTable() {
    ...
    for (var i = tras.length - 1; i >= 0; i--) {
        tras[i].onclick = (function (trNode) {
            return function () {
                var key = trNode.id;
                var day = trNode.getElementsByTagName("td")[0].innerHTML.substring(0, 10)
                var op = document.documentElement.scrollTop;
                $tbody.removeChild(trNode);
                delete searchHistory[day][key];
                (function (op) {
                    setTimeout(function () {
                        document.documentElement.scrollTop = op;
                    }, 10);
                })(op);
            }
